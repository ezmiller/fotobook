...
var fotobook = {
	...
	init: function() {
		...
		jQuery.post('admin-ajax.php', 'action=fotobook&get_sessions=true', function(response) {
	...
	linkNewAccount: function(message) {
		jQuery.post('admin-ajax.php', 'action=fotobook&destroy_session=true', function(response) {
			if ( response.indexOf('http') >= 0 ) { // The function returned a login url
