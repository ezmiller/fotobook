<?php
/*
Plugin Name: Fotobook Redux
Plugin URI: 
Description: Fotobook allows you to import Facebook photo galleries directly into WordPress.  <strong>Requires PHP 5. If you update this plugin you'll lose the pages functionality.</strong>
Author: Aaron Harp (modified by Ethan Miller)
Author URI: http://www.aaronharp.com/ http://tintinnabuleur.bcbc.co.uk
Version: 3.2.2
*/
...
// This is TEMPORARYcode just to prevent wordpress from looking for an update for the plugin
// since the plugin is still linked to the non-maintained version in the WP Repo.
// Code source: http://www.htmlgraphic.com/remove-update-checks-for-a-wordpress-plugin/
//
 ...
 * load_facebook_API
 * 
 * Loads the facebook api object if the user is working on the 
 * options pages. Is set to run with the hook 
 * 'admin_enqueue_scripts'. 
 */
function load_facebook_API ($hook_suffix) {
	if ( $hook_suffix == 'fotobook/options-fotobook.php' || $hook_suffix == 'fotobook/manage-fotobook.php' ) {
		global $fb_facebook;
		$fb_appId			= get_option('fb_app_id');
		$fb_appSecret		= get_option('fb_app_secret');
		$fb_facebook = new FacebookAPI($fb_appId, $fb_appSecret);
 	}
}
add_action('admin_enqueue_scripts', 'load_facebook_API');
...
function fb_ajax_handler() {
	...
	if(isset($_POST['destroy_session'])) {
		...
		// Load up a new facebook api; it will pick up the persistent data from the cookie.
		$fb_appId			= get_option('fb_app_id');
		$fb_appSecret		= get_option('fb_app_secret');
		$facebook = new FacebookAPI($fb_appId, $fb_appSecret);

		// Now destroy the session, so that the state object is refreshsed.
		$facebook->destroy_session();
		
		echo $facebook->get_login_url(); // Return a new login url.
	}

	// handle request for sessions
	if (isset($_POST['get_sessions'])) {
		...
		// Return the sessions.
		echo json_encode($facebook->get_sessions());  // returns an array of sessions or false if none.
...
function fb_parent_dropdown( $default = 0, $parent = 0, $level = 0 ) {
	...
	if ( $items ) {
		foreach ( $items as $item ) {
			...
			echo "\n\t<option value='$item->ID'$current>$pad " . esc_html($item->post_title) . "</option>";
...
function fb_display_manage_list($message = '') {
	...
	<ul id="fb-manage-list">
		...
		?>
		<li class="<?php echo $class ?>" id="album=<?php echo $album['aid'] ?>">
...
function fb_widget_init() {
	...
	wp_register_sidebar_widget('fotobook-photos-widget' , 'Fotobook Photos', 'fb_photos_widget');
	wp_register_widget_control(array('Fotobook Photos', 'widgets'), 'fb_photos_widget_control', 300, 150);
	wp_register_sidebar_widget('fotobook-albums-widget', 'Fotobook Albums', 'fb_albums_widget');
	wp_register_widget_control(array('Fotobook Albums', 'widgets'), 'fb_albums_widget_control', 300, 150);
